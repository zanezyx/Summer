...
public class UpdateManager {
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case DOWN_ERROR:
				// 下载apk失败
				Toast.makeText(mContext.getApplicationContext(), "下载新版本失败", 1)
						.show();
				// LoginMain();
				break;
			}
	...
	public void checkVersionUpdate(boolean tag)
	{
		try {
			if(SettingInfo.getInstance().latestVersionCode==null)
			{
				if(tag)
				{
					Toast.makeText(mContext.getApplicationContext(), "获取不到最新版本信息",1).show();
				}
				return;
			}
			if(!SettingInfo.getInstance().latestVersionCode.equals(getVersionName()))
			{
				...
				if(tag)
				{
					Toast.makeText(mContext.getApplicationContext(), "当前版本是最新版本",1).show();
				}
			}
		} catch (Exception e) {
